		...
		public static List<double [ ]> ContourPolar( List<double [ ]> src )
		{
			...
			for ( int i = 0 ; i < spaceDatas [ 0 ].Length ; i++ )
			{
				...
				while ( running )
				{
					...
					var l1 = thlist.Select( x => x != null ? Math.Abs(startTh - (double)x) : double.MaxValue).ToList(); // 제일 처음의 두께와 가장 가까운 것ㅁ만 찾아감. 범주조건 삭제. 무조건 최소 L1거리로만 이동하게됨. => 나선형 모양으로 수렴 안하게됨 실패 . 
					if ( pastmovement > -1 )
