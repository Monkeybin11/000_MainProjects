...
namespace ThicknessAndComposition_Inspector_IPS_Core
{
	public static class Mapping2
	{
		...
		public static void LoadMapdata()
		{
			...
			var thetasplited =  InterpolationRecurTheta( total ,3 );
		...
		// 세타는 무조건 올라가는 각도, 로우는 자유 롭게 한칸씩 이동. 이동은 현재의 높이와 오차가 가장 적은 쪽으로 이동하게 된다. 
		// 엔드 조건은 , 1 원래 자리 , 2.최외각 
		public static List<double [ ]> ContourPolar( List<double [ ]> src )
		{
			...
			for ( int i = 0 ; i < spaceDatas [ 0 ].Length ; i++ )
			{
				...
				var updw = GetClass(claslist , res.Th);
				if ( updw == null ) break;
				...
				while ( running )
				{
					...
					var thlist = dirtlist.Select( x => spaceDatas.Pick( nb [ x ] )?.Th ).ToList();
					...
					if ( pastmovement > -1 )
						l1 [ pastmovement ] = double.MaxValue;
